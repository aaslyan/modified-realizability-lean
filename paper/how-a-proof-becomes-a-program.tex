...
code with no human cleverness in between.
...
started from --- always, eventually, hits exactly $0$.
...
surprising. Table~\ref{tab:realizers} collects them --- the guiding question
...
continuous} --- redeemed two definitions on.
...
appears is the Sierpi\'nski triangle. There is no gap between the picture and
the theorem: every cell of the drawing is one instance of the proved
disjunction, its value filled in by the extracted realizer's own tag.
...
change. This is the one-dimensional Sperner lemma, the discrete ancestor of the
intermediate value theorem. The fragment proves it by ordinary induction,
...
This is where Act~I's promissory note --- that ordinary induction is not enough
--- comes due. Induct on $m$, or $s$, or the value $\good(m,s)$, and you fail:
the value goes \emph{up} at almost every step, so no natural-number quantity
decreases as the sequence runs. What decreases is not a natural number at all.
...
consequence of ordinary logic and ordinary induction. To make it mechanical the
...
\begin{tikzpicture}[node distance=5mm]
  \node[boxb](a){$\epsz$};
  \node[box,below=of a](b){$\omega^2$};
  \node[box,below=of b](c){$\omega$};
  \node[boxa,below=of c](d){$5$};
  \node[boxa,below=of d](e){$4$};
  \node[boxa,below=of e](f){$3$};
  \node[boxg,below=of f](g){$0$};
  \foreach \x/\y in {a/b,b/c,c/d,d/e,e/f,f/g} \draw[flow](\x)--(\y);
  \node[figcap,right=3mm of a]{start (an ordinal below $\epsz$)};
  \node[figcap,right=3mm of d,align=left]{each Goodstein step descends,};
  \node[figcap,right=3mm of e,align=left]{even as the \emph{number} grows};
  \node[figcap,right=3mm of g]{must reach the bottom};
\end{tikzpicture}
\caption{Well-foundedness in one picture: the ordinal assigned to each state
strictly descends at every step, and a descending chain of ordinals cannot go
on forever, so the process terminates --- however large the underlying numbers
become.}
\label{fig:ordinals}
\end{figure}
...
growing \emph{more} heads than it lost. The Kirby--Paris theorem says Hercules
wins anyway: every battle terminates. The reason is Goodstein's in disguise ---
...
Curry--Howard correspondence, both decades established. The reader was told as
