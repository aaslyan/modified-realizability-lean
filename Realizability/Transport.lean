...
noncomputable def liftR : (φ : Formula) → {m : ℕ} →
...
noncomputable def dropR : (φ : Formula) → {m : ℕ} →
...
noncomputable def liftIter (φ : Formula) : (k : ℕ) → {m : ℕ} →
...
noncomputable def dropIter (φ : Formula) : (k : ℕ) → {m : ℕ} →
...
noncomputable def famOf (φ : Formula) {m : ℕ} (x : PureType (m + 1)) :
    (n : ℕ) → PureType (n + 1) :=
  fun n =>
    if h : m ≤ n then
